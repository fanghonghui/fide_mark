  ...
  <div class="app-container">
    <el-card class="box-card search-box">
      <el-form :inline="true" :model="formData" class="demo-form-inline">
        ...
        <el-form-item>
          <div class="sub-title">销售经理</div>
          <el-select size="small" clearable v-model="formData.referrer" placeholder="全部">
            <el-option v-for="item in saleManagersOptions" :key="item.managerUuid" :label="item.managerName" :value="item.managerUuid">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
    ...
    <el-card class="box-card">
      <div class="data-total">
        <span class="pdr25">已申请：{{totalData.applyCount}}笔</span>
        <span class="pdr25">已分配：{{totalData.dispathCount}}笔</span>
      ...
      <el-table :data="tableData" @expand="expandDetails" :row-class-name="getRowClass" border style="width: 100%">
        ...
        <el-table-column type="expand" width="15" >
          <template scope="scope">
            <el-table :data="scope.row.turnPushOrgs" class="hide-expand" :row-class-name="tableRowClassName" border style="width: 100%">
              ...
              <el-table-column prop="referrerName"></el-table-column>
              <el-table-column width="91">
                <template scope="scope">
        ...
        <el-table-column prop="referrerName" label="销售经理"></el-table-column>
          ...
          <template scope="scope">
            <el-row>
              <el-col style="margin-top:5px;">
                ...
                <el-button size="small" class="mb5" type="primary" @click="doDistribute(scope.row)">分配</el-button>
                <el-button size="small" class="mb5" type="primary" @click="checkRecord(scope.row, scope.$index)">审批记录</el-button>
              </el-col>
            </el-row>
          </template>
    ...
    <el-dialog title="转推" :visible.sync="turnPush" class="turnPush">
      <table class="table">
        <thead>
          <tr>
            <th class="w50 title-th" style="padding: 0 19px;"></th>
            <th>可转推机构</th>
            <th>产品</th>
            <th>额度</th>
            <th>利率</th>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="confirmTurnPush">确 定</el-button>
      </span>
    </el-dialog>
    ...
    <!--分配-->
    <el-dialog class="distribute-modal" :visible.sync="distributeModal">
      <el-row :gutter="20" class="mb15">
        <el-col :span="16">
          <el-select filterable clearable v-model="managerUuid" placeholder="全部">
            <el-option v-for="item in saleManagersOptions" :key="item.managerUuid" :label="item.managerName" :value="item.managerUuid">
            </el-option>
          </el-select>
          <el-button type="primary" icon="search" @click="selectManagers">搜 索</el-button>
        </el-col>
        <el-col :span="8" style="text-align: right;">
          <el-button type="primary" @click="confirmDistribute">确 定</el-button>
          <el-button @click="distributeModal = false">取 消</el-button>
        </el-col>
      </el-row>

      <el-card class="box-card">
        <div slot="header">销售经理</div>

        <div class="clearfix">
          <el-radio v-for="item of distributionData" class="radio clearfix fl" v-model="selectedManagerUuid" :label="item.managerUuid"
            :value="item.managerUuid" :key="item.managerUuid">
            <div class="person-image fl">
              <img v-if="item.managerPic" :src="item.managerPic" />
              <span v-else>未上传</span>
            </div>
            <div class="fl">
              <p class="mb15 font-bold">姓名：{{item.managerName}}</p>
              <p class="mb15">电话：{{item.managerMobile || '暂无'}}</p>
            </div>
            </el-radio>
        </div>
      </el-card>
  ...
  export default {
    ...
    methods: {
      ...
      getApiOptions() {
        ...
        // 销售经理
        this.http.smallPost('queryAllSeller', {}).then(res => {
          if (res && res.data) {
            this.saleManagersOptions = res.data
            this.distributionData = res.data
            this.saleManagersData = res.data
          }
        })
      ...
      fetchData(pageNum) {
        ...
        this.http.post('approvalData', this.filterParams(params)).then(res => {
          if (res && res.data) {
            var data = res.data
      ...
      // 分配
      doDistribute(row) {
        this.selectedManagerUuid = row.referrer
        this.distributeModal = true
        this.distributionApplyUuid = row.applyUuid
      },

      // 搜索销售经理
      selectManagers() {
        var managersData = this.saleManagersData
        if (this.managerUuid) {
          for (var i in managersData) {
            if (managersData[i].managerUuid === this.managerUuid) {
              var selectedData = []
              selectedData.push(managersData[i])
            }
          }
          this.distributionData = selectedData
        } else {
          this.distributionData = managersData
        }
      },
      ...
      confirmDistribute() {
        var params = {
          managerUuid: this.selectedManagerUuid,
          applyUuid: this.distributionApplyUuid
        }
        this.http.post('confirmDistribute', params).then(res => {
          if (res) {
